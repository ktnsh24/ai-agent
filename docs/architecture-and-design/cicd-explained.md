...
> `Dockerfile` + `.github/workflows/` — three GitHub Actions workflows that lint, test, build, and (manually) deploy the agent to AWS ECS Fargate or Azure Container Apps.
...
|----------|---------|--------|-----------|
| `ci.yml` | `push` to `main`/`develop`, `pull_request` to `main` | Lint → Test → Docker Build | Automated pipeline who runs the report card on every shift change before letting anyone merge. |
| `deploy-aws.yml` | `workflow_dispatch` (manual, env = `dev` or `stg`) | OIDC login → ECR build & push → `terraform apply` → ECS service update | Robot hand on the AWS side — only acts when an operator pushes the deploy button. |
| `deploy-azure.yml` | `workflow_dispatch` (manual, env = `dev` or `stg`) | Azure login → ACR build & push → `terraform apply` → Container App update | Same robot hand on the Azure side — also strictly manual to keep cloud spend on a leash. |
...
| `test` | `ubuntu-latest` + Python 3.12 + Poetry | `poetry install` → `poetry run pytest tests/ -v` with `CLOUD_PROVIDER=local` | Pure-Python tests pass with the SQLite + local-provider backend; no cloud credentials needed in CI | Dry-run shift in a local test environment — courier, tool registry, and SQLite store each tested without a real LLM. |
| `docker` | `ubuntu-latest` | `docker build -t ai-agent:ci .` | The `Dockerfile` still produces a runnable image with the current `pyproject.toml` | Build the dispatch-desk uniform from the latest pattern and confirm it actually fits the staff. |
...
6. aws ecs update-service --force-new-deployment
        --cluster ai-agent-${env}
        --service ai-agent
```

The `environment: aws-${env}` line binds the job to a GitHub Environment (`aws-dev` or `aws-stg`), so required reviewers and restricted secrets all kick in before step 1 runs.
...
| Image registry | ECR | ACR | Two locker rooms, one per depot — the hand drops the new uniform in whichever the operator chose. |
...
| LLM provider keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.) | **Not** in CI — injected at runtime via Terraform / cloud secret manager | The running container | Permission-slip stamp lives in the dispatch shed's safe; the robot hand never carries it. |

There is no static cloud credential anywhere in the repo. OIDC is the only auth path the deploy workflows use.
...
There is intentionally no `prod` environment in the workflow inputs — only `dev` and `stg` are available. A production rollout requires deliberately editing the workflow, a friction-on-purpose design that matches the €5/month budget guard.
...
| Test regression | `test` job red on PR | Reproduce locally with `poetry run pytest -v`; SQLite backend means no external services needed | Dry-run shift exposed a misbehaving courier; rehearse in the local environment until it passes. |
| Docker build fail | `docker` job red, usually after a `pyproject.toml` change | `docker build -t ai-agent:dev .` locally, fix dependency pin | Tailor's pattern broke after a fabric change; cut a sample uniform and verify it fits before pushing. |
...
A failed deploy never leaves a half-deployed image without a Terraform record, because `terraform apply` runs *before* the runtime update — if Terraform fails, the running task definition is unchanged.

---

## 🚚 Courier Explainer

CI/CD is the **automated pipeline**. On every code push it runs a fixed report-card routine — lint (Ruff), test (`pytest` with local SQLite backend), and image build (`docker build`). A red mark anywhere stops the merge.

Deploys are deliberately a separate, **manual** lever. An operator hands the pipeline a deploy ticket that says either "AWS dev/stg" or "Azure dev/stg". The hand:
...
4. tells the running agent service to roll over to the new image.
