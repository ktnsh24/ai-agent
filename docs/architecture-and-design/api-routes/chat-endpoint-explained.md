...
> "How much does a flight to Paris cost right now, and convert that to GBP?"

This traces one complete request end-to-end across all four steps with concrete values.
...
No `conversation_id` — this is a brand new thread.

---

### Step 1 — Conversation creation

No `conversation_id` in the request → the store creates a new conversation:

- `id`: `"a3f7c2b1d4e8"` (generated by the store)
- `title`: `"How much does a flight to Pa"` (first 50 chars of the message)
- `created_at`: now

User message is written to the store **before the graph runs**:
...
### Step 2 — Agent graph execution (2 iterations)

**Initial state built before the graph runs:**

```
messages: [
  SystemMessage("You are a helpful assistant. Tools available: web_search, calculator"),
  HumanMessage("How much does a flight to Paris cost right now, and convert that to GBP?")
]
iterations: 0
max_iterations: 5
```
...
LLM receives the 2-message list. Decides it needs a web search.

Returns:
```
AIMessage(tool_calls=[{id: "tc_001", name: "web_search", arguments: {"query": "London to Paris flight price today"}}])
```

`should_continue()`: last message is `AIMessage` with `tool_calls` → route to `tools`.

**Iteration 1 — `tools` node**

`ToolNode` executes `web_search("London to Paris flight price today")`.

Tavily returns:
```
ToolMessage(tool_call_id: "tc_001", content: "Flights from London to Paris: from £89 one-way on EasyJet, from £120 on British Airways")
```

Message list now has 4 messages. Loop back to `agent`.

**Iteration 2 — `agent` node**

LLM receives the 4-message list. The price is already in GBP (`£89`) — no conversion needed. Decides to answer directly.

Returns:
```
AIMessage(content: "Based on current search results, flights from London to Paris start from £89 one-way on EasyJet. That price is already in GBP — no conversion required.")
```

`should_continue()`: last message is `AIMessage` with no `tool_calls` → route to `end`.

**Graph exits. Iterations used: 2 of 5.**

---

### Step 3 — Conversation store write

Assistant message written to the store:
...
`_extract_tool_calls()` walks the finished message list and produces:
  ...
  "tool_calls_made": [
    {
      "id": "tc_001",
      "name": "web_search",
      "arguments": {"query": "London to Paris flight price today"},
      "result": "Flights from London to Paris: from £89 one-way on EasyJet..."
    }
  ],
  "iterations": 2,
  "status": "COMPLETE",
...
`total_tokens: 312` is the rough estimate (total chars in all messages ÷ 4) — not a real count from the Bedrock API.

---

### Follow-up — using the calculator too

If the user now sends a second message **on the same conversation**:

```json
POST /v1/chat
{
  "message": "If I travel 3 times, what's the total cost?",
  "conversation_id": "a3f7c2b1d4e8"
}
```

**Step 1** loads the prior two messages from the store (user + assistant).

**Initial state:**
```
messages: [
  SystemMessage(...),
  HumanMessage("How much does a flight to Paris cost right now..."),
  AIMessage("...start from £89 one-way..."),   ← loaded from store
  HumanMessage("If I travel 3 times, what's the total cost?")
]
```

The LLM has full context — it knows `£89` from the prior turn. It calls `calculator("89 * 3")` in iteration 1, gets `"267"`, then answers `"£267 total for 3 flights"` in iteration 2.

This is the `add_messages` annotation at work — without it, the LLM would lose the `£89` context between turns and would have to search again.
